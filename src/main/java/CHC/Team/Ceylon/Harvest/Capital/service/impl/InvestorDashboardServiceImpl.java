...
public class InvestorDashboardServiceImpl implements InvestorDashboardService {
        ...
        // Validate investor balance ────────────────────────────────────
        @Override
        public void validateInvestorBalance(Long investorId, long investmentAmount) throws Exception {
                Wallet wallet = walletRepository.findByUserUserId(investorId)
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Wallet not found for investor: " + investorId));

                if (wallet.getBalance().longValue() < investmentAmount) {
                        throw new Exception("Insufficient wallet balance: " + wallet.getBalance());
                }
        }
